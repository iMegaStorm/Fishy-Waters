...
int CURRENT_SCREEN = INSTRUCTIONS_MENU_SCREEN; //Using current screen in order to switch to another screen
...
Sprite &grass_32, Sprite &grass_33, Sprite &bush_45, Sprite &tree_46, Sprite &tree_47, Sprite &tree_48, Sprite &tree_49, Sprite &rock_50, Sprite &shipRed, Sprite &shipRedDead, Sprite &shipBlue, Sprite &shipBlueDead, Sprite &shipGreen, Sprite &shipGreenDead, Sprite &pirateShip, Sprite &pirateShipDamaged, Sprite &pirateShipDead, Sprite &cannonBall, Sprite &gameScreenBackground, Sprite &instructionsMenuBackground, Sprite &fishyWorldTitle, Sprite &instructionsTitle, Sprite &selectBoat, Sprite &sardine, Sprite &tuna, Sprite &pufferFish, Sprite &trout, Sprite &clownFish, Sprite &shop,
...
//Local Variables
	...
	Sprite shipType[6] = {shipRed, shipBlue, shipGreen, shipRedDead, shipBlueDead, shipGreenDead};
	...
	if(sec % secToAdd == secToAdd - 1 && noFishes < 15)
	{
		...
		window.draw(Arial);
			...
			{
				shipType[1].setPosition(posBoat.x*64+32 +startJ, posBoat.y*64-32 +startI);
				shipType[1].setScale(1,1);
				//window.draw(shipType[1]);
			}
			else if (PLAYER_SHIP == GREEN_SHIP)
			{
				shipType[2].setPosition(posBoat.x*64+32 +startJ, posBoat.y*64-32 +startI);
